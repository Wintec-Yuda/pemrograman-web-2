...
define('PW','');
define('DB','3A');
$koneksi_server=MySQLi_connect(HOST,USER,PW,DB);
if (!$koneksi_server){
	die('Gagal koneksi ke server '.mysqli_error($koneksi_server));
}
//echo "Berhasil ke server<br>";
$koneksi_database=mysqli_select_db($koneksi_server,DB);
if (!$koneksi_database){
	die('Tidak bisa akses '.DB.' '.mysqli_error($koneksi_database));
}
//echo "Berhasil ke database<br>";
		...
		<div class="container">
			...
			<div class="navbar-collapse collapse">
				<ul class="nav navbar-nav pull-right mainNav">
					<?php
						$sql='SELECT * FROM tbmenu WHERE parent_id=0 
						ORDER BY urutan ASC';
						$result = mysqli_query($koneksi_server,$sql);
						while ($hasil=mysqli_fetch_array($result)){
							$id_main=$hasil['id'];
							$s='SELECT * FROM tbmenu WHERE parent_id='.$id_main;
							$res = mysqli_query($koneksi_server,$s);
							$n=mysqli_num_rows($res);
							if ($n==0){
								echo "<li><a href=''>".$hasil['menu']."</a></li>";
							}else{
								echo "<li><a href='' class='dropdown-toggle' data-toggle='dropdown'>".$hasil['menu']." <b class='caret'></b></a>";
								echo "<ul class='dropdown-menu'>";
										while ($hsl = mysqli_fetch_array($res)){
											echo "<li><a href='#'>".$hsl['menu']."</a></li>";
										}																
									echo "</ul>";
								echo "</li>";
							}
